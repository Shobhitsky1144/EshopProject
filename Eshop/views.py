...
def contact(request):
    cats=Category.objects.all()
    data1=Contact_Us.objects.all().order_by("-id")
    if request.method=="POST":
        ...
        return render(request,"contact.html",{"status":res,"data":data1,"category":cats})
    return render(request,'contact.html',{"data":data1,"category":cats})
...
def register(request):
    if request.method=="POST":
        ...
        usr=User.objects.create_user(un,e,pas)
        ...
        reg.save()
        return render(request,"register.html",{"status":"Mr/Miss. {} your Account created Successfully".format(first)})
...
def user_login(request):
    if request.method=="POST":
        ...
        user = authenticate(username=un,password=pwdd)
        if user:
            login(request,user)
...
def sell_dashboard(request):
    return render(request,"sell_dashboard.html")
@login_required(login_url="/")
